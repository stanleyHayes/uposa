...
export default function MemberProfilePage() {
  ...
  useEffect(() => {
    ...
    membersApi.getById(id)
      .then((res) => setMember(res.data.data || MOCK_MEMBERS.find((m) => m.id === id) || null))
      .catch(() => setMember(MOCK_MEMBERS.find((m) => m.id === id) || null))
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {member.areaOfExpertise.map((area) => (
                      <span key={area} className="badge badge-outline badge-sm">{area}</span>
                    ))}
                  </div>
                </div>
              )}
