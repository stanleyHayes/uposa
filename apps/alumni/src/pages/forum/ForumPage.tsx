...
export default function ForumPage() {
  ...
  useSocketEvent('forum:newPost', (newPost: any) => {
    ...
    <PageTransition>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 mb-6">
        ...
        <div className="relative flex-1 max-w-xs">
          ...
          <input type="text" className="input input-sm input-bordered w-full pl-9" placeholder="Search discussions..." value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
        <button className="btn btn-primary btn-sm gap-1.5 ml-auto" onClick={() => setModalOpen(true)}>
        ...
        <div className="space-y-2">
          {filtered.map((post, i) => (
            <ScrollReveal key={post.id} delay={i * 0.02}>
              <Link to={`/forum/${post.slug}`} className="card bg-base-100 shadow-[0_2px_8px_rgba(0,27,80,0.06)] hover:bg-base-200/50 transition-colors">
