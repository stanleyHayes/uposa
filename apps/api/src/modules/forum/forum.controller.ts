...
export async function adminTogglePinPostHandler(req: Request, res: Response): Promise<void> {
  const { id } = req.params;
  const post = await adminTogglePinPost(id);
  successResponse(res, `Post ${post.isPinned ? 'pinned' : 'unpinned'}`, post);
}

export async function adminToggleLockPostHandler(req: Request, res: Response): Promise<void> {
  const { id } = req.params;
...
export async function adminDeleteCommentHandler(req: Request, res: Response): Promise<void> {
  const { id } = req.params;
